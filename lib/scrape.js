 ...
 *           paginate?: boolean, maxPages?: number }} opts
 */
export async function scrapeOverstocks({
  ...
  try {
    ...
    // a single infinite-scroll page; quarterly paginates via ?page=N.
    const seen = new Map();
    let priceReady = false;
    let pagesCovered = 0;
    const maxP = paginate ? Math.max(1, maxPages) : 1;

    for (let pageNo = 1; pageNo <= maxP; pageNo++) {
      ...
      if (!paginate) break;
      // Stop when a page is empty, adds nothing new (past the last page — Magento
      // clamps ?page beyond the end to the last page, yielding duplicates), or
      // we're low on time budget.
      if (!gotGrid || pageProducts.length === 0 || newCount === 0) {
        log('No new products — end of pagination.');
        break;
      }
      if (msLeft() < 14000) {
        log(`Stopping pagination at page ${pageNo} to stay within time budget (more pages may exist).`);
        break;
      }
    }
    ...
    log(
      `Extracted ${products.length} products (${withPrice} with a price) across ${pagesCovered} page(s).`
    );
