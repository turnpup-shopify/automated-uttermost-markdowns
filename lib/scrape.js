...
/**
 * Main entry point. Logs in, loads the given promotion page, and extracts
 * SKU/price/compare-at. Both promo pages share identical markup, so only the
 * target `url` differs.
 * @param {{ url?: string, debug?: boolean, screenshot?: boolean,
 *           paginate?: boolean, maxPages?: number,
 *           startPage?: number, seedProducts?: Array }} opts
 *   startPage/seedProducts let a run RESUME a previous incomplete paginated
 *   scrape: start at `startPage` and merge into `seedProducts` already gathered.
 */
export async function scrapeOverstocks({
  url,
  debug = false,
  screenshot = false,
  paginate = false,
  ...
  try {
    ...
    // Accumulate unique products by SKU across one or more pages. Overstocks is
    // a single infinite-scroll page; quarterly paginates via ?page=N. Seed with
    // products from a prior incomplete run so we can resume where we left off.
    const seen = new Map();
    for (const p of seedProducts || []) {
      if (p && p.sku) seen.set(p.sku, p);
    }
    const seededCount = seen.size;

    let priceReady = false;
    let pagesCovered = 0; // pages fetched THIS run
    let complete = !paginate; // single-page sources are always complete
    let nextPage = 1; // where the NEXT run should resume (1 = full restart)

    const start = paginate ? Math.max(1, startPage || 1) : 1;
    const maxP = paginate ? Math.max(start, maxPages) : 1;
    if (seededCount) log(`Resuming from page ${start} with ${seededCount} products already gathered.`);

    for (let pageNo = start; pageNo <= maxP; pageNo++) {
      const pageUrl = buildPageUrl(overstocksUrl, pageNo);
      log(`Navigating${paginate ? ` page ${pageNo}` : ''}: ${pageUrl}`);
      ...
      const gotGrid = await page
        ...
        .then(() => true)
        .catch(() => {
          log('WARNING: product root .item-root-Chs not found on this page.');
          return false;
        });

      await loadAllProducts(page, { loadMoreSelector, log, msLeft });

      // Prices hydrate a beat AFTER the card renders (authenticated GraphQL).
      const priceWait = Math.min(paginate ? 8000 : 12000, msLeft() - 6000);
      const thisPriceReady =
        priceWait > 1000
          ? await page
              .waitForFunction(
                () => {
              ...
              .catch(() => false)
          : false;
      priceReady = priceReady || thisPriceReady;

      const pageProducts = await page.evaluate(extractProductsInPage);
      let newCount = 0;
      for (const p of pageProducts) {
        if (!p.sku) continue;
        if (!seen.has(p.sku)) newCount += 1;
        seen.set(p.sku, p);
      }
      pagesCovered = pageNo;
      log(`Page ${pageNo}: ${pageProducts.length} products, ${newCount} new (total ${seen.size}).`);

      if (!paginate) {
        complete = true;
        break;
      }
      // Reached the end: a page is empty or adds nothing new (Magento clamps
      // ?page beyond the last page, yielding duplicates).
      if (!gotGrid || pageProducts.length === 0 || newCount === 0) {
        complete = true;
        nextPage = 1;
        log('No new products — end of pagination reached.');
        break;
      }
      // Out of time budget: stop and flag where to resume next run.
      if (msLeft() < 14000) {
        complete = false;
        nextPage = pageNo + 1;
        log(`Time budget reached at page ${pageNo}. Will resume from page ${nextPage} next run.`);
        break;
      }
      // Hit the page cap without reaching the end — treat as incomplete.
      if (pageNo === maxP) {
        complete = false;
        nextPage = maxP + 1;
        log(`Hit maxPages (${maxP}) without reaching the end. Resume from page ${nextPage} next run.`);
      }
    }
    ...
    log(
      `Extracted ${products.length} products (${withPrice} with a price); ${pagesCovered} page(s) this run; ` +
        `${complete ? 'COMPLETE' : `INCOMPLETE (resume at page ${nextPage})`}.`
    );
...
// Append ?page=N (N>=2) to a promotion URL, preserving any existing query.
