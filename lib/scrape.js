...
async function login(page, { log }) {
  ...
  const passwordSelector =
    process.env.UTTERMOST_PASSWORD_SELECTOR || 'input[type="password"]';
  ...
  log('Waiting for the password field (login-form anchor).');
  const gotPassword = await page
    .waitForSelector(passwordSelector, { timeout: 15000, visible: true })
    .then(() => true)
    .catch(() => false);
  diag.passwordFieldFound = gotPassword;

  if (!gotPassword) {
    // Capture what forms DO exist so we can see where the login form lives.
    diag.formsOnPage = await page.evaluate(() =>
      [...document.querySelectorAll('form')].slice(0, 4).map((f) => ({
        id: f.id,
    ...
    diag.error = 'No password field found on the login page — see formsOnPage / resolvedUrl.';
    log(diag.error);
    return diag;
  }

  // Tag the correct email/password/submit within the password field's own form.
  const formInfo = await page.evaluate(
    (emailSelOverride) => {
      ...
      let emailEl = null;
      if (emailSelOverride) emailEl = form.querySelector(emailSelOverride);
      if (!emailEl) {
        emailEl =
  ...
  if (formInfo.emailFound) await page.type('[data-scrape-email]', email, { delay: 15 });
  await page.type('[data-scrape-pw]', password, { delay: 15 });

  log(`Submitting credentials (button: "${formInfo.submitText || 'n/a'}").`);
  const clicked = await page
